...
bool Application::Initialise_DX10(HINSTANCE _hInstance)
{
	...
	m_pObj_Sphere->SetPosition({ 0.0f, 3.0f, 7.0f });
	...
	m_pObj_Capsule->SetPosition({ -10.0f, 3.0f, 6.0f });
	...
	m_pObj_Pyramid->SetPosition({ 10.0f, 3.0f, 7.0f });
...
void Application::Render()
{
	...
	if (m_pDX10_Renderer != 0)
	{
		...
		TLight* m_pArrLights = m_pDX10_Renderer->GetActiveLights();
		std::map<std::string, DX10_TextureResource*>* pMapShadows = m_pDX10_Renderer->GetActiveShadowMaps();
		int lightCount = m_pDX10_Renderer->GetLightCount();

		std::map<std::string, DX10_TextureResource*>::iterator iterShadows = pMapShadows->begin();
		for (int i = 0; i < lightCount; i++)
		{
			iterShadows->second->StartRender();

			m_pShader_Shadow->CreateShadowMaps(m_pObj_Floor, &m_pArrLights[i]);
			m_pShader_Shadow->CreateShadowMaps(m_pObj_Sphere, &m_pArrLights[i]);
			m_pShader_Shadow->CreateShadowMaps(m_pObj_Capsule, &m_pArrLights[i]);
			m_pShader_Shadow->CreateShadowMaps(m_pObj_Pyramid, &m_pArrLights[i]);
		
			iterShadows->second->EndRender();
			iterShadows++;
		}
		...
		m_pObj_Floor->Render(TECH_LITTEX_SHADOW);
		m_pObj_Sphere->Render(TECH_LITTEX_SHADOW);
		m_pObj_Capsule->Render(TECH_LITTEX_SHADOW);
		m_pObj_Pyramid->Render(TECH_LITTEX_SHADOW);
...
void Application::HandleInputCamera()
{
	// Camera Controls
