...
bool DX10_Renderer::Initialise(int _clientWidth, int _clientHeight, HWND _hWND)
{
	...
	pLight->dir_spotPow = D3DXVECTOR4(0.0f, -1.0f, 0.0f, 0.0f);
...
void DX10_Renderer::ShutDown()
{
	...
	// Delete the Graphics memory stored as Shadow maps
	std::map<std::string, DX10_TextureResource*>::iterator iterShadows = m_shadowMaps.begin();
	while (iterShadows != m_shadowMaps.end())
	{
		ReleasePtr(iterShadows->second);
		iterShadows++;
	}
...
bool DX10_Renderer::onResize()
{
	...
	D3DXMatrixOrthoLH(&m_matOrtho, 30.0f, 30.0f, 1.0f, 100.0f);
...
void DX10_Renderer::RestoreDefaultRenderStates()
{
...
bool DX10_Renderer::AddLight(std::string _lightName, TLight* _light)
{
	...
	// Create a pair for the new shadow map
	DX10_TextureResource* pShadowMap = new DX10_TextureResource();
	VALIDATE(pShadowMap->Initialise(m_pDX10Device, 0, 1024, 1024, TRT_SHADOW));
	std::pair<std::string, DX10_TextureResource*> pairShadow(_lightName, pShadowMap);

	// Insert the pair and validate the insertion
	VALIDATE(m_mapLights.insert(pairLight).second);
	VALIDATE(m_shadowMaps.insert(pairShadow).second);
...
void DX10_Renderer::RemoveLight(std::string _lightName)
{
	...
	// Release the Shadow Map for that light
	ReleasePtr(m_shadowMaps.find(_lightName)->second);
	m_shadowMaps.erase(_lightName);
...
std::map<std::string, DX10_TextureResource*>* DX10_Renderer::GetActiveShadowMaps()
{
	//// Delete the pointer to current array of shadow maps
	//ReleasePtr(m_pArrShadowMaps);
	//
	//m_pArrShadowMaps = new DX10_TextureResource[m_shadowMaps.size()];
	//std::map<std::string, DX10_TextureResource*>::iterator iterShadows = m_shadowMaps.begin();
	//int index = 0;
	//while (iterShadows != m_shadowMaps.end())
	//{
	//	m_pArrShadowMaps[index] = *iterShadows->second;
	//
	//	index++;
	//	iterShadows++;
	//}
