...
float3 ParallelLight(SurfaceInfo v, Light L, float3 eyePos, float shadowFactor = 1.0f)
{
	...
	if (diffuseFactor > 0.0f)
	{
		...
		litColor += shadowFactor * (diffuseFactor * (float3)(v.diffuse * L.diffuse));
		litColor += shadowFactor * (specFactor * (float3)(v.spec * L.spec));
	}
