...
VS_OUT_SHADOW VS_Shadow(VS_IN _inputVS)
{
	VS_OUT_SHADOW outputVS;
	...
	// TO DO CAL
	outputVS.projTexCoord = mul(float4(_inputVS.position, 1.0f), g_matWorld);
	outputVS.projTexCoord = mul(outputVS.positionH, g_matViewLight);
	outputVS.projTexCoord = mul(outputVS.positionH, g_matProjLight);
...
float CalcShadowFactor(float4 projTexC)
{
	...
	float s0 = g_mapShadow.Sample(g_shadowSam, projTexC.xy).r;
	float s1 = g_mapShadow.Sample(g_shadowSam, projTexC.xy + float2(SMAP_DX, 0)).r;
	float s2 = g_mapShadow.Sample(g_shadowSam, projTexC.xy + float2(0, SMAP_DX)).r;
	float s3 = g_mapShadow.Sample(g_shadowSam, projTexC.xy + float2(SMAP_DX, SMAP_DX)).r;
...
float4 PS_Shadow(VS_OUT_SHADOW _inputPS) : SV_Target
{
	float4 diffuse = g_mapDiffuse.Sample(g_triLinearSam, _inputPS.texCoord);
	...
	float3 normal = 2 * _inputPS.normal - 1;
	normal = normalize(normal);

	float shadowFactor = CalcShadowFactor(_inputPS.projTexCoord);
	...
	for (int i = 0; i < g_lightCount; i++)
	{
		if (g_light[i].active == true)
		{
			if (g_light[i].type == 0)
			{
				litColor += ParallelLight(surface, g_light[i], g_eyePosW, shadowFactor);
			}
			else if (g_light[i].type == 1)
			{
				litColor += PointLight(surface, g_light[i], g_eyePosW);
